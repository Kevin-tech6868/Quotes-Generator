...
def clean_quote_text(text: str) -> str:
    """
    Clean and normalize quote text with strict truncation at the first full stop.
    """
    # Remove extra whitespace
    text = ' '.join(text.split())
    
    # Truncate at the first full stop
    if '.' in text:
...
def fetch_quotes_from_serpapi(prompt: str) -> List[Tuple[str, str]]:
    ...
    if len(quotes_and_authors) < 5:
        general_fallbacks = [
            ("Failure is the stepping stone to success.", "Unknown"),
